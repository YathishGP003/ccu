...
import com.renovo.bacnet4j.cache.CachePolicies;
import com.renovo.bacnet4j.cache.RemoteEntityCache;
...
import com.renovo.bacnet4j.event.DeviceEventHandler;
import com.renovo.bacnet4j.event.ExceptionDispatcher;
...
import com.renovo.bacnet4j.obj.mixin.CovContext;
import com.renovo.bacnet4j.persistence.IPersistence;
import com.renovo.bacnet4j.persistence.NullPersistence;
import com.renovo.bacnet4j.service.VendorServiceKey;
...
import com.renovo.bacnet4j.service.unconfirmed.UnconfirmedCovNotificationRequest;
...
import com.renovo.bacnet4j.util.RemoteDeviceDiscoverer;
import com.renovo.bacnet4j.util.RemoteDeviceFinder;
import com.renovo.bacnet4j.util.RemoteDeviceFinder.RemoteDeviceFuture;

import utils.warp.WarpScheduledExecutorService;
import utils.warp.WarpUtils;
...
public class LocalDevice {
     ...
     */
    private long timeoutDeviceRetention = 60000;
    ...
    public LocalDevice(final int deviceNumber,final String deviceName,final Transport transport) {
        this.transport = transport;
        transport.setLocalDevice(this);
        afterInstatiation(deviceNumber,deviceName);
    }

    private void afterInstatiation(final int deviceNumber,final String deviceName) {
        try {
            // Initialize the device object.
            new DeviceObject(this, deviceNumber,deviceName);
    ...
    public synchronized LocalDevice initialize(final RestartReason lastRestartReason) throws Exception {
        ...
        if (getInstanceNumber() == ObjectIdentifier.UNINITIALIZED) {
            ...
            while (remaining > 0) {
                ...
                final List<Integer> idList = IntStream.range(from, from + rangeSize).boxed().collect(Collectors.toList());
    ...
    public boolean checkObjectByID(final int instanceID) {
        for (final BACnetObject obj : localObjects) {
    ...
    public void removeObjectByID(final int instanceID) throws BACnetServiceException {
        final BACnetObject obj = getObjectByID(instanceID);
        if (obj != null) {
            localObjects.remove(obj);
    ...
    public RemoteDeviceDiscoverer startRemoteDeviceDiscovery(final Consumer<RemoteDevice> callback) {
        final Consumer<RemoteDevice> cachingCallback = new Consumer<RemoteDevice>() {
			
			@Override
			public void accept(RemoteDevice t) {
