...
import com.renovo.bacnet4j.obj.DeviceObject;
import com.renovo.bacnet4j.obj.FileObject;
import com.renovo.bacnet4j.obj.fileAccess.StreamAccess;
import com.renovo.bacnet4j.service.Service;
import com.renovo.bacnet4j.service.confirmed.AtomicReadFileRequest;
import com.renovo.bacnet4j.service.confirmed.AtomicWriteFileRequest;
import com.renovo.bacnet4j.service.confirmed.CreateObjectRequest;
import com.renovo.bacnet4j.service.confirmed.ReadPropertyMultipleRequest;
...
import com.renovo.bacnet4j.service.confirmed.WritePropertyMultipleRequest;
...
public class DefaultReinitializeDeviceHandler implements ReinitializeDeviceHandler {
    ...
    public static boolean isReInitiated = false;
    ...
    protected void coldstart(final LocalDevice localDevice, final Address from) throws BACnetErrorException {
        try {
            System.out.println("Bacnet "+"Cold Start-"+"localDevice:"+localDevice+" fromAddress:"+from);
            LocalDevice reInitialized = reInitialize(localDevice);
            localDevice.initialize(RestartReason.coldstart);
            System.out.println("Bacnet "+"Cold Start-"+"localDevice:"+localDevice+" reInitialized:"+reInitialized.isInitialized());
            if(reInitialized.isInitialized()){
                sendWhois(reInitialized);
            }
            //reInitialized.initialize(RestartReason.coldstart);
    ...
    protected void warmstart(final LocalDevice localDevice, final Address from) throws BACnetErrorException {
        try {
            System.out.println("Bacnet "+"Warm Start-"+"localDevice:"+localDevice+" fromAddress:"+from);
            LocalDevice reInitialized = reInitialize(localDevice);
            System.out.println("Bacnet "+"Warm Start-"+"localDevice:"+localDevice+" reInitialized:"+reInitialized.isInitialized());
            ...
            new java.util.Timer().schedule(
                    new java.util.TimerTask() {
                        ...
                        public void run() {
                            try {
                                localDevice.terminate();
                                System.out.println("Bacnet "+"Warm Start-"+"localDevice:"+localDevice+" reInitialized:"+reInitialized.isInitialized()+" terminated");
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                        }
            ...
            new java.util.Timer().schedule(
                    new java.util.TimerTask() {
                        ...
                        public void run() {
                            try {
                                localDevice.initialize(RestartReason.warmstart);
                                System.out.println("Bacnet "+"Warm Start-"+"localDevice:"+localDevice+" reInitialized:"+localDevice.isInitialized()+" re-started");
                                if(localDevice.isInitialized()){
                                    sendWhois(localDevice);
                                }
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
    ...
    public static LocalDevice reInitialize(LocalDevice localDevice){
        LocalDevice newLocalDevice = null;
        try {
            Transport defaultTransport = new DefaultTransport(localDevice.getNetwork());
            newLocalDevice = new LocalDevice(localDevice.getInstanceNumber(), localDevice.get(PropertyIdentifier.objectName).toString(), defaultTransport);
            defaultTransport.terminate();

            defaultTransport.setLocalDevice(newLocalDevice);
            isReInitiated = true;
        } catch (Exception e) {
            e.printStackTrace();
            isReInitiated = false;
    ...
    public boolean isIsReInitiated(){
        return isReInitiated;
    }

    public void sendWhois(LocalDevice localDevice){
        if(localDevice.isInitialized()) {
            localDevice.sendLocalBroadcast(new WhoIsRequest());
            localDevice.sendLocalBroadcast(new IAmRequest(new ObjectIdentifier(ObjectType.device,localDevice.getInstanceNumber()), localDevice.get(PropertyIdentifier.maxApduLengthAccepted), localDevice.get(PropertyIdentifier.segmentationSupported),localDevice.get(PropertyIdentifier.vendorIdentifier)));
        }
    }
}
