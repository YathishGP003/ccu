...
import com.renovo.bacnet4j.obj.mixin.CovReportingMixin;
import com.renovo.bacnet4j.obj.mixin.HasStatusFlagsMixin;
import com.renovo.bacnet4j.obj.mixin.ObjectIdAndNameMixin;
import com.renovo.bacnet4j.obj.mixin.PropertyListMixin;
...
import com.renovo.bacnet4j.service.acknowledgement.GetEnrollmentSummaryAck.EnrollmentSummary;
import com.renovo.bacnet4j.service.acknowledgement.GetEventInformationAck.EventSummary;
import com.renovo.bacnet4j.service.confirmed.GetEnrollmentSummaryRequest.AcknowledgmentFilter;
import com.renovo.bacnet4j.service.confirmed.GetEnrollmentSummaryRequest.EventStateFilter;
import com.renovo.bacnet4j.service.confirmed.GetEnrollmentSummaryRequest.PriorityFilter;
...
import com.renovo.bacnet4j.type.constructed.RecipientProcess;
...
public class BACnetObject {
    ...
    public BACnetObject(final LocalDevice localDevice, final ObjectIdentifier id, final String name) {
        ...
        addMixin(new ReadOnlyPropertyMixin(this, PropertyIdentifier.statusFlags, PropertyIdentifier.eventState,
                PropertyIdentifier.priorityArray,PropertyIdentifier.relinquishDefault,PropertyIdentifier.propertyList,
                PropertyIdentifier.currentCommandPriority,PropertyIdentifier.eventTimeStamps,PropertyIdentifier.eventMessageTexts,
                PropertyIdentifier.maxApduLengthAccepted,PropertyIdentifier.segmentationSupported,PropertyIdentifier.notifyType,
                PropertyIdentifier.eventEnable,PropertyIdentifier.bufferSize,PropertyIdentifier.logDeviceObjectProperty,
                PropertyIdentifier.totalRecordCount,PropertyIdentifier.inactiveText, PropertyIdentifier.activeText));
    }
    ...
    public BACnetObject writeProperty(final ValueSource valueSource, final PropertyValue value)
        ...
        if (PropertyIdentifier.objectIdentifier.equals(pid))
            throw new BACnetServiceException(ErrorClass.property, ErrorCode.writeAccessDenied);
        if (PropertyIdentifier.objectName.equals(pid))
            throw new BACnetServiceException(ErrorClass.property, ErrorCode.writeAccessDenied);
        if (PropertyIdentifier.eventState.equals(pid))
            throw new BACnetServiceException(ErrorClass.property, ErrorCode.writeAccessDenied);
        /*if (PropertyIdentifier.ackedTransitions.equals(pid))
            throw new BACnetServiceException(ErrorClass.property, ErrorCode.writeAccessDenied);*/
        if (PropertyIdentifier.changeOfStateTime.equals(pid))
            throw new BACnetServiceException(ErrorClass.property, ErrorCode.writeAccessDenied);
        if (PropertyIdentifier.changeOfStateCount.equals(pid))
            throw new BACnetServiceException(ErrorClass.property, ErrorCode.writeAccessDenied);
        if(PropertyIdentifier.utcOffset.equals(pid)){
            ...
            if (pin == null) {
                ...
                if (def == null) {
                    ...
                    if (pdef.isArray()) {
                        ...
                        if (!(valueToWrite instanceof SequenceOf) && !(valueToWrite instanceof DateRange))
                            throw new BACnetServiceException(ErrorClass.property, ErrorCode.invalidDataType);
