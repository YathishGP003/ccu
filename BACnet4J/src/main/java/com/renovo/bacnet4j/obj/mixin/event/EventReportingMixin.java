...
import com.renovo.bacnet4j.service.unconfirmed.UnconfirmedEventNotificationRequest;
...
abstract public class EventReportingMixin extends AbstractMixin {
    ...
    protected boolean validateProperty(final ValueSource valueSource, final PropertyValue value)
        ...
        if (value.getPropertyIdentifier().isOneOf( //
                //PropertyIdentifier.eventDetectionEnable, // Todo for testing purpose
    ...
    protected void doStateTransition(final EventState toState) {
        ...
        final Boolean eventDetectionEnable = get(PropertyIdentifier.eventDetectionEnable); // Todo Testing purpose we have added this check

        // Do we need to send any notifications?
        if (eventEnable.contains(toState) && eventDetectionEnable.booleanValue()) {
            ...
            final SequenceOf<UnsignedInteger> priority = nc.get(PropertyIdentifier.priority);
    ...
    private void sendNotifications(final SequenceOf<Destination> recipientList, final TimeStamp timeStamp,
            final BACnetObject nc, final SequenceOf<UnsignedInteger> priority, final EventState toState,
