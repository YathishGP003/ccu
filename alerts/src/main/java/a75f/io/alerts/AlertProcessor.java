...
public class AlertProcessor
{
    ...
    AlertProcessor(Context c, AlertsService alertsService) {
        ...
        // Todo: we should probably fetch all site definitions at every restart.
        // (Currently its problematic b/c we'd fetch alerts at least twice simultaneously, possible threading errors)
    ...
    public void fetchPredefinedAlerts() {
        ...
        fetchDisposable =
            alertsService.getSiteDefinitions(siteId)
                ...
                .subscribe(
                        this::handleRetrievedAlerts,
                        error -> { CcuLog.e("CCU_ALERTS", "Unexpected error fetching or parsing site definitions.", error); }
                );
    }
    
    private void handleRetrievedAlerts(ArrayList<AlertDefinition> alertDefs) {

        this.predefinedAlerts = alertDefs;
        //add wifi signal alert definition

        CcuLog.d("CCU_ALERTS", "Fetched " + predefinedAlerts.size() + " Predefined alerts");
        if (predefinedAlerts.size() > 0)
    ...
    public List<Alert> getAllAlertsNotInternal(){
        ...
        alertQuery.notEqual(Alert_.mSeverity, Alert.AlertSeverity.INTERNAL_ERROR.ordinal());
        alertQuery.notEqual(Alert_.mSeverity, Alert.AlertSeverity.INTERNAL_WARN.ordinal());
