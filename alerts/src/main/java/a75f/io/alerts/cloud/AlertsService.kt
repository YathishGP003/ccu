...
interface AlertsService {
   ...
   /**
    * Get all the predefined alert definitions.
    */
   @GET("/definitions?size=100&sortField=alert.mSeverity&sortDirection=DESC")
   fun getPredefinedDefinitions(): Single<DefinitionsResponse>

   /**
    * Get all alert definitions for a site, predefined and custom.
    */
   @GET("/definitions/sites/{siteId}")
...
class ServiceGenerator {
   ...
   /** A weird sort of lazy init wherein we can swap in a new value -- a new service -- when the the
    * baseUrl changes.
    */
   lateinit var alertsService: AlertsService

   /** We change the base URL on the fly in local environment, like when we change local IP address betw/ home and office */
   fun updateAlertsUrl(newBaseUrl: String): AlertsService {
      alertsService = serviceWithUrl(newBaseUrl)
      return alertsService
   }

   private fun serviceWithUrl(baseUrl: String): AlertsService {

      return createRetrofit(
         baseUrl,
         HttpHeaders(contentType = "application/json", apiKey = BuildConfig.ALERTS_API_KEY)
      )
