...
import a75f.io.alerts.Conditional
   ...
   fun retrieveAlertDefs() {
      ...
      alertManager.offsetCounter
         .forEach { keyValue ->
            ...
            if (alertDef == null) {
               val key = titleMapping.keys.first { title.startsWith(it) }
...
@ColorRes fun Alert.AlertSeverity.color(): Int {
   return when (this) {
      Alert.AlertSeverity.SEVERE -> R.color.severity_severe
      Alert.AlertSeverity.MODERATE -> R.color.severity_moderate
      Alert.AlertSeverity.LOW -> R.color.severity_low
      Alert.AlertSeverity.INFO -> R.color.severity_info
      Alert.AlertSeverity.INTERNAL_SEVERE -> R.color.severity_severe
      Alert.AlertSeverity.INTERNAL_MODERATE -> R.color.severity_moderate
      Alert.AlertSeverity.INTERNAL_LOW -> R.color.severity_low
      Alert.AlertSeverity.INTERNAL_INFO -> R.color.severity_info
   }
}
