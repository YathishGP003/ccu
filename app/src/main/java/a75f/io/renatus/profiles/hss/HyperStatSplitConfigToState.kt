...
fun configSensorAddress(config: HyperStatSplitProfileConfiguration, state: HyperStatSplitState) {
...
fun configUniversalIn(config: HyperStatSplitProfileConfiguration, state: HyperStatSplitState) {
...
fun configAnalogOut(config: HyperStatSplitProfileConfiguration, state: HyperStatSplitState) {
    ...
    if (config is HyperStatSplitCpuProfileConfiguration && state is HyperStatSplitCpuState) {
        updateAnalogOutDynamicConfig(config, state)
    }
}

fun configRelay(config: HyperStatSplitProfileConfiguration, state: HyperStatSplitState) {
...
fun configMisc(config:HyperStatSplitProfileConfiguration, state: HyperStatSplitState) {
...
fun updateAnalogOutDynamicConfig(config: HyperStatSplitCpuProfileConfiguration, state: HyperStatSplitCpuState) {
    ...
    state.stagedFanVoltages = StagedFanVoltage(
        ...
        config.stagedFanVoltages.heatStage1Voltage.currentVal.toInt(), config.stagedFanVoltages.coolStage1Voltage.currentVal.toInt(),
        config.stagedFanVoltages.heatStage2Voltage.currentVal.toInt(), config.stagedFanVoltages.coolStage2Voltage.currentVal.toInt(),
        config.stagedFanVoltages.heatStage3Voltage.currentVal.toInt(), config.stagedFanVoltages.coolStage3Voltage.currentVal.toInt()
    )
