...
open class HyperStatViewModel(application: Application) : AndroidViewModel(application) {
    ...
    var testSignal by mutableStateOf(false)
    ...
    fun sendTestSignal(relayIndex: Int = -1, analogOut: Int = -1, status: Double = 0.0) {
        ...
        if (relayIndex != -1) {
            when (relayIndex) {
                1 -> device.relay1.writePointValue(status)
                2 -> device.relay2.writePointValue(status)
                3 -> device.relay3.writePointValue(status)
                4 -> device.relay4.writePointValue(status)
                5 -> device.relay5.writePointValue(status)
                6 -> device.relay6.writePointValue(status)
            }
        }
        if (analogOut != -1) {
            when (analogOut) {
    ...
    fun applyCopiedConfiguration() {
        ...
        else if (CopyConfiguration.getSelectedProfileType() == ProfileType.HYPERSTAT_FOUR_PIPE_FCU){
            viewState.value = HyperStatViewStateUtil.pipe4ConfigToState(CopyConfiguration.getCopiedConfiguration() as HsPipe4Configuration)
        }
    ...
    fun getAnalogOutValue(analogOutIndex: Int): Double? {
        ...
        if (equipRef != null) {
            val device = getHyperStatDomainDevice("", equipRef!!)
            when (analogOutIndex) {
                1 -> value = device.analog1Out.readHisVal() / 10
                2 -> value = device.analog2Out.readHisVal() / 10
                3 -> value = device.analog3Out.readHisVal() / 10
            }
        }
        return value
    }
    ...
    fun getRelayStatus(relayIndex: Int): Boolean {
        if (equipRef != null) {
            ...
            return when (relayIndex) {
                ...
                5 -> device.relay5.readPointValue() > 0
                6 -> device.relay6.readPointValue() > 0
                else -> false
            }
        }
        return false
