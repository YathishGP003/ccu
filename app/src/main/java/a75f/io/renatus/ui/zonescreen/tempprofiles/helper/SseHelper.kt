...
import a75f.io.api.haystack.Tags.SSE
...
import a75f.io.renatus.ui.zonescreen.tempprofiles.view.showSseDetailedView
...
class SseHelper(
    ...
    val equip: SseEquip = Domain.getDomainEquip(equipMap.id) as SseEquip
    ...
    companion object {
        fun create(
            ...
            return SseHelper(equipMap, profileType)
        }
    }

    fun profileName() = SSE.uppercase()  + " ( " + equipMap.group + " )"
    ...
    private fun loadSseDefaults() {
        detailViewItems.clear()
        val dischargeAirFlowView = getDischargeAirTempView()
        detailViewItems[dischargeAirFlowView.id.toString()] = dischargeAirFlowView
    ...
    private fun getDischargeAirTempView(): DetailedViewItem {
        var dischargePoint = equip.dischargeAirTemperature.readHisVal().toString()
