...
public class Initializer {
     ...
     * (default {@code false}). 
     */
    public static final Boolean SHOW_INITIALIZED = false;
    ...
    public void loadClass(Class<?> cls) {
        try {
            ...
            Log.d("Initialize", "Class " + cls + " not found.");
        }
    }
    ...
    public boolean initializeLoadedClasses() {
        ...
        while (true) {
            ...
            if (classes == null) {
                Log.d("Initializer", "Automatic class initialization not supported.");
                return false;
            }
            ...
            for (int i = nbrClassesInitialized; i < classes.length; i++) {
                ...
                try {
                    if (SHOW_INITIALIZED)
                        Log.d("Initialize ", cls.getName());
                    Class.forName(cls.getName(), true, classLoader);
                } catch (Throwable error) {
                    isInitializationSuccessful = false;
                    Log.w("Class ", cls.getName() + " cannot be initialized.");
                }
            }
            nbrClassesInitialized = classes.length;
        }
        Log.d("Initializer", "Initialization of " + nbrClassesInitialized +
