...
public final class FractalTableImpl implements Serializable, Cloneable {
    ...
    /**
     * An array of data elements or fractal tables (recursion). Data length
     * varies from 2 to BASE_CAPACITY_MAX
     */
    private Object[] data;
    ...
    private FractalTableImpl allocate(int i) {
        FractalTableImpl fractal = new FractalTableImpl(shift - SHIFT, new Object[1 << SHIFT], 0);
        data[i] = fractal;
        return fractal;
    }
    ...
    public Object get(int index) {
        ...
        return (shift == 0) ? fractal : ((FractalTableImpl) fractal).get(index + offset);
    }
    ...
    /**
     * Shifts the specified elements(]last - length, last] modulo capacity) one
     * position to the left. No shift if length (modulo capacity) is zero.
     */
    public void shiftLeft(Object inserted, int last, int length) {
        ...
        if (shift == 0) {
            ...
            if ((head <= tail) && ((head >> shift) == (tail >> shift))) {
    ...
    public FractalTableImpl upsize() {
        if (data.length >= BASE_CAPACITY_MAX) { // Creates outer fractal.
            ...
            FractalTableImpl table = new FractalTableImpl(shift, new Object[data.length << 1], 0);
            copyTo(table);
            return table;
        }
    }
