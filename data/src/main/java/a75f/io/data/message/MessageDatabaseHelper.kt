...
class MessageDatabaseHelper @Inject constructor (private val messagesDb: RenatusDatabase) : DatabaseHelper {
    ...
    override fun getAllMessages(): LiveData<List<Message>> =
        messagesDb.messageDao().getAllMessages()
    ...
    override suspend fun insertAll(messages: List<Message>) =
        messagesDb.messageDao().insertAll(messages)

    override suspend fun deleteAll(messages: List<Message>) =
        messagesDb.messageDao().deleteAll(messages)

    override suspend fun updateAll(message: List<Message>) =
        messagesDb.messageDao().updateAll(message)
    ...
    override suspend fun getMessageById(messageId: String): Message =
        messagesDb.messageDao().getMessageById(messageId)

    override suspend fun getAllExpiredMessages(expiryTime: Long): List<Message> =
        messagesDb.messageDao().getAllExpiredMessages(expiryTime)
}