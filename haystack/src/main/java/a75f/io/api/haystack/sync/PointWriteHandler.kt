...
class PointWriteHandler {
    ...
    fun processPointWrites(pointWriteList: List<Map.Entry<String, Array<HDict>>>) {
    ...
    private fun processBatch(pointWriteBatch: List<Array<HDict>>, ccuHsApi: CCUHsApi) {
        ...
        for (array in pointWriteBatch) {
            ...
            for (dict in array) {
                listOfPointArrayDict.add(dict)
            }
        }
        val pointWriteGrid = HGridBuilder.dictsToGrid(listOfPointArrayDict.toTypedArray())
        ...
        CcuLog.d(PointWriteCache.POINT_WRITE_TAG, "PointWriteMany Request : "+HZincWriter.gridToString(pointWriteGrid))
