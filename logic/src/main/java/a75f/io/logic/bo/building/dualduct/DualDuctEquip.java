...
public class DualDuctEquip {
            ...
            switch (op.getPort()) {
                case ANALOG_OUT_ONE:
                    if (config.getAnalogOut1Config() == DualDuctAnalogActuator.NOT_USED.getVal()) {
                        ...
                        device.analog1In.setEnabled(false);
                    } else {
                        ...
                        if (config.getAnalogOut1Config() == DualDuctAnalogActuator.COOLING.getVal()) {
                            ...
                            device.analog1In.setType(config.getAnalog1OutAtMinDamperCooling()+"-"+
                                    config.getAnalog1OutAtMaxDamperCooling()+"v");
                            ...
                            device.analog1In.setType(config.getAnalog1OutAtMinDamperHeating()+"-"+
                                    config.getAnalog1OutAtMaxDamperHeating()+"v");
                        }
                ...
                case ANALOG_OUT_TWO:
                    if (config.getAnalogOut2Config() == DualDuctAnalogActuator.NOT_USED.getVal()) {
                        ...
                        device.analog2In.setEnabled(false);
                        ...
                        if (config.getAnalogOut2Config() == DualDuctAnalogActuator.COOLING.getVal()) {
                            ...
                            device.analog2In.setType(config.getAnalog2OutAtMinDamperCooling()+"-"+
                                    config.getAnalog2OutAtMaxDamperCooling()+"v");
                            ...
                            device.analog2In.setType(config.getAnalog2OutAtMinDamperHeating()+"-"+
                                    config.getAnalog2OutAtMaxDamperHeating()+"v");
                        }
                        device.analog2Out.setPointRef(logicalPointId);
                        device.analog2In.setPointRef(damperFeedbackID);
                    }
                    break;
            }
        }
    ...
    private void createEquipOperationPoints(String siteRef, String equipDis, String roomRef, String floorRef, String tz) {
        ...
        Point equipScheduleStatus = new Point.Builder()
                                            ...
                                            .setRoomRef(roomRef)
                                            .setFloorRef(floorRef).setHisInterpolate("cov")
                                            .addMarker("scheduleStatus").addMarker("logical").addMarker("dualDuct").addMarker("zone").addMarker("writable")
                                            .setGroup(String.valueOf(nodeAddr))
                                            .setTz(tz)
                                            .setKind(Kind.STRING)
                                            .build();
    ...
    private void createOutputConfigPoints(String siteRef,
        ...
        Point thermistor2OutputConfig = new Point.Builder()
                                                ...
                                                .addMarker("sp").addMarker("his")
                                                .setEnums("coolingSupplyAirflow, heatingSupplyAirflow")
                                                .setGroup(String.valueOf(nodeAddr))
                                                .setTz(tz)
                                                .build();
    ...
    private String createAnalog2LogicalPoint(String siteRef,
        ...
        if (config.getAnalogOut2Config() == DualDuctAnalogActuator.COOLING.getVal()) {
            ...
            if (config.getAnalogOut1Config() == DualDuctAnalogActuator.HEATING.getVal() ||
                config.getAnalogOut1Config() == DualDuctAnalogActuator.COMPOSITE.getVal()) {
                
                HashMap<Object, Object> heatingDamperPoint = CCUHsApi.getInstance().readEntity("point and dualDuct " +
                                                                "and heating and damper and pos and cmd and group == \""+nodeAddr+"\"");
                heatingDamperPosId = heatingDamperPoint.get("id").toString();
            } else {
    ...
    private void updateAnalog1LogicalPoint(String siteRef,
        ...
        if (config.getAnalogOut1Config() == DualDuctAnalogActuator.NOT_USED.getVal()) {
            ...
            String coolingDamperPosId = createAnalog1LogicalPoint (siteRef, equipDis, roomRef, floorRef, tz, config);
            ...
            String damperFeedbackID = createFeedbackPoint(CCUHsApi.getInstance(),nodeAddr,equipDis,equipRef,siteRef,
                    roomRef,floorRef,tz,"analog1");
            SmartNode.setPointEnabled(nodeAddr, Port.ANALOG_IN_ONE.toString(), true);
            SmartNode.updatePhysicalPointRef(nodeAddr, Port.ANALOG_IN_ONE.toString(), damperFeedbackID);
            SmartNode.updatePhysicalPointType(nodeAddr, Port.ANALOG_IN_ONE.toString(),
                    config.getAnalog1OutAtMinDamperCooling()+"-"+config.getAnalog1OutAtMaxDamperCooling()+"v");
            
        } else if (config.getAnalogOut1Config() == DualDuctAnalogActuator.HEATING.getVal()) {
            
            String heatingDamperPosId = createAnalog1LogicalPoint (siteRef, equipDis, roomRef, floorRef, tz, config);
            SmartNode.updatePhysicalPointType(nodeAddr, Port.ANALOG_OUT_ONE.toString(),
    ...
    private void updateAnalog2LogicalPoint(String siteRef,
        ...
        if (config.getAnalogOut2Config() == DualDuctAnalogActuator.NOT_USED.getVal()) {
            ...
            String coolingDamperPosId = createAnalog2LogicalPoint (siteRef, equipDis, roomRef, floorRef, tz, config);
            ...
            String damperFeedbackID = createFeedbackPoint(CCUHsApi.getInstance(),nodeAddr,equipDis,equipRef,siteRef,
                    roomRef,floorRef,tz,"analog2");
            SmartNode.setPointEnabled(nodeAddr, Port.ANALOG_IN_TWO.toString(), true);
            SmartNode.updatePhysicalPointRef(nodeAddr, Port.ANALOG_IN_TWO.toString(), damperFeedbackID);
            SmartNode.updatePhysicalPointType(nodeAddr, Port.ANALOG_IN_TWO.toString(),
                    config.getAnalog1OutAtMinDamperCooling()+"-"+config.getAnalog1OutAtMaxDamperCooling()+"v");
            ...
            String compositeDamperPosId = createAnalog2LogicalPoint (siteRef, equipDis, roomRef, floorRef, tz, config);
            SmartNode.updatePhysicalPointType(nodeAddr, Port.ANALOG_OUT_TWO.toString(),
                                              OutputAnalogActuatorType.ZeroToTenV.displayName);
            SmartNode.setPointEnabled(nodeAddr, Port.ANALOG_OUT_TWO.toString(), true);
    ...
    public static String createFeedbackPoint(
        ...
        HashMap<Object, Object> feedbackPoint =
                CCUHsApi.getInstance().readEntity
                        (analogTag+" and in and point and damper and dualDuct and sensor and equipRef == \"" + equipRef+
                                "\"");
        if(!feedbackPoint.isEmpty()) {
            ccuHsApi.deleteEntityTree(Objects.requireNonNull(feedbackPoint.get("id")).toString());
        }

        // Create damper feedback point for analog in
        Point damperFeedback = new Point.Builder()
                .setDisplayName(siteDis+"-damper"+analogTag+"Feedback")
                .setEquipRef(equipRef)
                .setSiteRef(siteRef)
                .setRoomRef(room)
                .setFloorRef(floor).setHisInterpolate("cov")
                .addMarker("damper").addMarker("dualDuct")
                .addMarker("temp").addMarker("sensor").addMarker("in").addMarker(analogTag)
    ...
    public void updateEquip(DualDuctProfileConfiguration config) {
        ...
        setHisVal("analog2 and output and type",config.getAnalogOut2Config());
        setConfigNumVal("th1 and output and type",config.getThermistor1Config());
        setHisVal("th1 and output and type",config.getThermistor1Config());
        setConfigNumVal("th2 and output and type",config.getThermistor2Config());
        setHisVal("th2 and output and type",config.getThermistor2Config());
        
        setConfigNumVal("analog1 and output and min and damper and heating and pos",config.getAnalog1OutAtMinDamperHeating());
        setHisVal("analog1 and output and min and damper and heating and pos",config.getAnalog1OutAtMinDamperHeating());
        setConfigNumVal("analog1 and output and max and damper and heating and pos", config.getAnalog1OutAtMaxDamperHeating());
        setHisVal("analog1 and output and max and damper and heating and pos", config.getAnalog1OutAtMaxDamperHeating());
        setConfigNumVal("analog1 and output and min and damper and cooling and pos", config.getAnalog1OutAtMinDamperCooling());
        setHisVal("analog1 and output and min and damper and cooling and pos", config.getAnalog1OutAtMinDamperCooling());
        setConfigNumVal("analog1 and output and max and damper and cooling and pos", config.getAnalog1OutAtMaxDamperCooling());
        setHisVal("analog1 and output and max and damper and cooling and pos", config.getAnalog1OutAtMaxDamperCooling());
        setConfigNumVal("analog2 and output and min and damper and heating and pos", config.getAnalog2OutAtMinDamperHeating());
        setHisVal("analog2 and output and min and damper and heating and pos", config.getAnalog2OutAtMinDamperHeating());
        setConfigNumVal("analog2 and output and max and damper and heating and pos", config.getAnalog2OutAtMaxDamperHeating());
        setHisVal("analog2 and output and max and damper and heating and pos", config.getAnalog2OutAtMaxDamperHeating());
        ...
        setConfigNumVal("enable and iaq",config.isEnableIAQControl() ? 1.0 : 0);
        setHisVal("enable and iaq",config.isEnableIAQControl() ? 1.0 : 0);
        
        setConfigNumVal("temperature and offset",config.getTemperatureOffset());
        setHisVal("temperature and offset",config.getTemperatureOffset());
