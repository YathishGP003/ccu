                        ...
                        if(roomTemp > averageDesiredTemp){
                           // state = COOLING;
                            ...
                            //if(hpuDevice.getStatus() != DEADBAND.ordinal())
                                hpuDevice.setStatus(COOLING.ordinal());
                        }
                        break;
                    case HEAT_ONLY:
                        if(roomTemp < averageDesiredTemp){
                            ...
                            //if(hpuDevice.getStatus() != DEADBAND.ordinal())
                                hpuDevice.setStatus(HEATING.ordinal());
                        }
                        break;
                    case OFF: {
                        handleFanStages(hpuEquip.getId(),node,fanSpeed,curHumidity);
    ...
    public void updateHumidityStatus(int fanStage2Type, Short addr,String equipId, double curValue, HashMap<String,Integer> relayStages){
        ...
        switch (fanRelayType){
            ...
            case HUMIDIFIER:
                ...
                double targetThreshold = CCUHsApi.getInstance().readPointPriorityValByQuery("point and standalone and target and humidity and equipRef == \"" + equipId + "\"");
                ...
                break;
            case DE_HUMIDIFIER:
                double targetDehumidityThreshold = CCUHsApi.getInstance().readPointPriorityValByQuery("point and standalone and target and dehumidifier and equipRef == \"" + equipId + "\"");
                if((curValue > 0) && occupied) {
