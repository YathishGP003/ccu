...
import a75f.io.logic.bo.building.statprofiles.util.pipe4UvTuners
...
class Pipe4UnitVentilatorProfile(private val equipRef: String, nodeAddress: Short) :
    ...
    override fun updateZonePoints() {
        ...
        resetEquip(hssEquip)
        val pipe4Tuners = pipe4UvTuners(hssEquip)
    ...
    private fun runTitle24Rule(config: Pipe4UVConfiguration) {
        ...
        fanEnabledStatus = config.isFanEnabled(
            config, HyperStatSplitConfiguration.HyperStatSplitControlType.FAN_ENABLED.name
        )
    ...
    private fun runSpecifiedAnalogFanSpeed(domainName: String?, config: Pipe4UVConfiguration) {
        if (domainName != null) {
            val analogOuts = config.getAnalogOutsConfigurationMapping()
            analogOuts.forEach { (enabled, association, port) ->
                if (enabled && Pipe4UvAnalogOutControls.values()[association] == Pipe4UvAnalogOutControls.FAN_SPEED) {
                    val voltage = config.getFanConfiguration(port)
                    val fanSignal = when (domainName) {
                        DomainName.fanLowSpeed -> voltage.fanAtLow.currentVal.toInt()
                        DomainName.fanMediumSpeed -> voltage.fanAtMedium.currentVal.toInt()
                        DomainName.fanHighSpeed -> voltage.fanAtHigh.currentVal.toInt()
                        else -> fanLoopOutput
                    }
