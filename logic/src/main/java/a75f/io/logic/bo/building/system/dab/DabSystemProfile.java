...
public abstract class DabSystemProfile extends SystemProfile
{
    ...
    {
            CCUHsApi hayStack = CCUHsApi.getInstance();
        Point relay1Op = new Point.Builder().setDisplayName(SystemTuners.getDisplayNameFromVariation(equipDis + "-" + loop + "LoopOutput"))
    ...
    public void addDabSystemTuners(String equipref)
    {
        ...
        Point analogFanSpeedMultiplier = new Point.Builder().setDisplayName(SystemTuners.getDisplayNameFromVariation(HSUtil.getDis(equipref) + "-" + "analogFanSpeedMultiplier")).setSiteRef(siteRef).setEquipRef(equipref).setHisInterpolate("cov").addMarker("system").addMarker("tuner").addMarker("dab").addMarker("writable").addMarker("his").addMarker("analog").addMarker("fan").addMarker("speed").addMarker("multiplier").addMarker("sp")
        ...
        Point humidityHysteresis = new Point.Builder().setDisplayName(SystemTuners.getDisplayNameFromVariation(HSUtil.getDis(equipref) + "-" + "humidityHysteresis")).setSiteRef(siteRef).setEquipRef(equipref).setHisInterpolate("cov").addMarker("system").addMarker("tuner").addMarker("dab").addMarker("writable").addMarker("his").addMarker("humidity").addMarker("hysteresis").addMarker("sp")
                .setUnit("%").setMinVal("0").setMaxVal("100").setIncrementVal("1").setTunerGroup(TunerConstants.DAB_TUNER_GROUP).setTz(tz).build();
        ...
        Point relayDeactivationHysteresis = new Point.Builder().setDisplayName(SystemTuners.getDisplayNameFromVariation(HSUtil.getDis(equipref) + "-" + "relayDeactivationHysteresis")).setSiteRef(siteRef).setEquipRef(equipref).setHisInterpolate("cov").addMarker("system").addMarker("tuner").addMarker("dab").addMarker("writable").addMarker("his").addMarker("relay").addMarker("deactivation").addMarker("hysteresis").addMarker("sp")
    ...
    private void addModeChangeHysteresisChangeOverTuner(String equipRef) {
        ...
        if (modeChangeOverHysteresisPoint.isEmpty()) {
            ...
            Point modeChangeoverHysteresis = new Point.Builder()
                                                 .setDisplayName(SystemTuners.getDisplayNameFromVariation(HSUtil.getDis(equipRef)+"-modeChangeoverHysteresis"))
                                                 .setSiteRef(siteRef)
    ...
    private void addStageUpTimerCounterTuner(String equipRef) {
        ...
        if (stageUpTimerCounterPoint.isEmpty()) {
            ...
            Point stageUpTimerCounter = new Point.Builder()
                                                 .setDisplayName(SystemTuners.getDisplayNameFromVariation(HSUtil.getDis(equipRef)+"-stageUpTimerCounter"))
                                                 .setSiteRef(siteRef)
                                                 .setEquipRef(equipRef)
                                                 .setHisInterpolate("cov")
                                                 .addMarker("tuner").addMarker("dab").addMarker("writable").addMarker("his")
    ...
    private void addStageDownTimerCounterTuner(String equipRef) {
        ...
        if (stageDownTimerCounterPoint.isEmpty()) {
            ...
            Point stageDownTimerCounter = new Point.Builder()
                                            .setDisplayName(SystemTuners.getDisplayNameFromVariation(HSUtil.getDis(equipRef)+"-stageDownTimerCounter"))
                                            .setSiteRef(siteRef)
                                            .setEquipRef(equipRef)
                                            .setHisInterpolate("cov")
                                            .addMarker("tuner").addMarker("dab").addMarker("writable").addMarker("his")
    ...
    protected void addUserIntentPoints(String equipref)
    {
        ...
        Point desiredCI = new Point.Builder().setDisplayName(SystemTuners.getDisplayNameFromVariation(equipDis + "-" + "desiredCI")).setSiteRef(siteRef).setEquipRef(equipref).setHisInterpolate("cov").addMarker("system").addMarker("userIntent").addMarker("writable").addMarker("ci").addMarker("desired").addMarker("sp").addMarker("his").setTz(tz).build();
        ...
        Point systemState = new Point.Builder().setDisplayName(SystemTuners.getDisplayNameFromVariation(equipDis + "-" + "conditioningMode")).setBacnetId(BacnetIdKt.CONDITIONINGMODEID).setBacnetType(BacnetUtilKt.MULTI_STATE_VALUE)
        ...
        Point compensateHumidity = new Point.Builder().setDisplayName(SystemTuners.getDisplayNameFromVariation(equipDis + "-" + "compensateHumidity")).setSiteRef(siteRef).setEquipRef(equipref).addMarker("system").setHisInterpolate("cov").addMarker("userIntent").addMarker("writable").addMarker("his").addMarker("compensate").addMarker("humidity").setEnums("false,true").setTz(tz).build();
    ...
    private void addEffectiveSatConditioningPoint(String equipRef, CCUHsApi hayStack) {
        if (hayStack.readEntity("system and effective and sat and conditioning").isEmpty()) {
            ...
            Point effectiveSatConditioning = new Point.Builder()
                    .setDisplayName(SystemTuners.getDisplayNameFromVariation((HSUtil.getDis(equipRef) + "-effectiveSatConditioning")))
