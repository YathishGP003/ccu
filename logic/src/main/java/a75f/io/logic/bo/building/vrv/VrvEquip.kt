...
class VrvEquip(hsApi : CCUHsApi,
    ...
    private fun createIduStatusPoints(
        ...
        val fanSpeedControl = Point.Builder()
            .setDisplayName(equip.displayName + "-fanSpeedControl")
            .setEquipRef(equip.id)
            .setSiteRef(equip.siteRef)
            .setRoomRef(roomRef)
            .setFloorRef(floorRef)
            .setHisInterpolate("cov")
            .addMarker("zone").addMarker("fanSpeedControl").addMarker("vrv")
            .addMarker("sp").addMarker("his")
            .setGroup(nodeAddr.toString())
            .setEnums("D,C,A")
            .setTz(equip.tz)
            .build()
        val fanSpeedControlId = hayStack.addPoint(fanSpeedControl)
        hayStack.writeHisValById(fanSpeedControlId, 0.0)
