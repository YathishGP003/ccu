...
public abstract class UsbSerialDevice implements UsbSerialInterface
{
	...
	public static UsbSerialDevice createUsbSerialDevice(UsbDevice device,
	...
	public static UsbSerialDevice createUsbSerialDevice(UsbDevice device,
	                                                    UsbDeviceConnection connection, int iface)
	{
		/*
		...
		public void run()
		{
			while (working.get())
			{
				    ...
				    request.getEndpoint().getDirection() == UsbConstants.USB_DIR_IN)
				{
					...
					if (isFTDIDevice())
					{
						((FTDISerialDevice) usbSerialDevice).ftdiUtilities
								.checkModemStatus(data); //Check the Modem status
	...
	protected class ReadThread extends Thread
	{
		...
		public void run()
		{
			...
			while (working.get())
			{
				...
				if (inEndpoint != null)
				{
					numberBytes = connection.bulkTransfer(inEndpoint, serialBuffer
							                                                  .getBufferCompatible(), SerialBuffer.DEFAULT_READ_BUFFER_SIZE, 0);
				}
				else
				{
					numberBytes = 0;
				...
				if (numberBytes > 0)
				{
					...
					{
						((FTDISerialDevice) usbSerialDevice).ftdiUtilities
								.checkModemStatus(dataReceived);
						if (dataReceived.length > 2)
						{
							dataReceived = ((FTDISerialDevice) usbSerialDevice).ftdiUtilities
									               .adaptArray(dataReceived);
